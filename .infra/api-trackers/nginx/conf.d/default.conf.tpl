...
{{ $PHP_FPM_DSN := default .Env.NGINX_PHP_FPM_DSN "php-fpm-trackers:9000" -}}
server {
    listen 444;
...
server {
    ...
    location / {
        ...
        return 404 '{"errors":[{"code":404,"title":"ResourceNotFoundException"}]}';
    }

    location ~ ^/(statusz|pingz)$ {
        allow 127.0.0.1;
        deny  all;
    ...
    location ~* ^/api/storage/map-settings {
        deny all;
        return 404;
    }

    location ~* ^/api/storage/map-setting {
        deny all;
        return 404;
    }

    location ~* ^/api/storage/mapsetting.* {
        {{- template "cors" . }}
