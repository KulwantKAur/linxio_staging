 ...
 * TrackerHistoryIO
 */
#[ORM\Table(name: 'tracker_history_io')]
#[ORM\Index(name: 'tracker_history_io_device_id_type_id_occurred_at_on_index', columns: ['device_id', 'type_id', 'occurred_at_on'])]
#[ORM\Index(name: 'tracker_history_io_vehicle_id_type_id_occurred_at_on_index', columns: ['vehicle_id', 'type_id', 'occurred_at_on'])]
#[ORM\Entity(repositoryClass: 'App\Repository\Tracker\TrackerHistoryIORepository')]
#[ORM\EntityListeners(['App\EventListener\Tracker\TrackerHistoryIOEntityListener'])]
class TrackerHistoryIO extends BaseEntity
{
    ...
    public const DEFAULT_EVENT_LOG_DETAILS = [
        'deviceId',
        'device',
        'vehicleId',
        'driverId',
     ...
     * @var int
     */
    #[ORM\Column(name: 'value_on', type: 'integer', nullable: false, options: ['default' => '1'])]
    private $valueOn = 1;

    /**
     * @var int
     */
    #[ORM\Column(name: 'value_off', type: 'integer', nullable: true)]
    private $valueOff;
    ...
    {
        return $this->getDriver() ? $this->getDriver()->getId() : null;
    }
     ...
     * @param int $valueOn
     */
    public function setValueOn(int $valueOn): self
    {
     ...
     * @param int $valueOff
     */
    public function setValueOff(int $valueOff): self
    {
     ...
     * @return ?int
     */
    public function getStatusIO(): ?int
    {
        if ($this->getValueOn() && !is_null($this->getValueOff())) {
            return $this->getValueOff();
        }

        return $this->getValueOn() ?? null;
    }
    ...
    public function fromTrackerHistory(TrackerHistory $trackerHistory): self
    {
        ...
        $this->setTrackerHistoryOn($trackerHistory);
        $this->setOccurredAtOn($trackerHistory->getTs());
    ...
    public function toArray(array $include = []): array
    {
        ...
        if (in_array('distance', $include, true)) {
            $data['distance'] = ($this->getTrackerHistoryOn()->getOdometer() &&
                $this->getTrackerHistoryOff() && $this->getTrackerHistoryOff()->getOdometer())
                ? $this->getTrackerHistoryOff()->getOdometer() - $this->getTrackerHistoryOn()->getOdometer()
                : null;
        }
        if (in_array('duration', $include, true)) {
            $data['duration'] = ($this->getOccurredAtOn() && $this->getOccurredAtOff())
                ? $this->getOccurredAtOff()->getTimestamp() - $this->getOccurredAtOn()->getTimestamp()
                : null;
        }

        if (in_array('statusIO', $include, true)) {
            $data['statusIO'] = $this->getStatusIO();
        }
