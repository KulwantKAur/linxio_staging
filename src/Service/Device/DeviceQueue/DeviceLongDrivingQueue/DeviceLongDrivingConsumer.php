...
class DeviceLongDrivingConsumer implements ConsumerInterface
{
    ...
    private const TAG_LONG_DRIVING = 'longDriving';
    ...
    public const ROUTING_KEY_PREFIX = 'long_driving_device_'; // should be equal to `routing_keys` of queues

    private const TTS_TEXT = 'You are required by law to pull over and rest. Please find a suitable location and rest.';
    ...
    public function execute(AMQPMessage $msg)
    {
        try {
            ...
            $event = $this->slaveEntityManager->getRepository(Event::class)
                ->getEventByName(Event::VEHICLE_LONG_DRIVING);
            ...
            try {
                ...
                foreach ($notifications as $ntf) {
                    $this->setRedisKey(self::TAG_LONG_DRIVING, $event, $device, $ntf);
                    $this->cacheData = $this->memoryDb->getFromJson($this->redisKey) ?: [];
                    $status = $this->getStatus($trackerData, self::TAG_LONG_DRIVING);
                    $this->cacheData = $this->updateCache(['status' => $status], self::TAG_LONG_DRIVING);
...
//        }
        ...
        $this->cacheData = $this->updateCache(['isTrigger' => true], self::TAG_LONG_DRIVING);
