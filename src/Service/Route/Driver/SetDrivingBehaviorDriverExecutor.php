...
final class SetDrivingBehaviorDriverExecutor implements SetDriverExecutorInterface
{
    private $em;
    ...
    public function execute($driverHistoryId): void
    {
        ...
        if ($driverHistory) {
            ...
            $iterator = $this->getDrivingBehaviorByVehicleAndStartedDateIterator(
     ...
     * @return iterable
     */
    private function getDrivingBehaviorByVehicleAndStartedDateIterator(Vehicle $vehicle, \DateTime $startedAt): iterable
    {
        return $this->em
            ->getRepository(DrivingBehavior::class)
            ->getDrivingBehaviorByVehicleAndStartedDateQb($vehicle, $startedAt)
            ->iterate();
    }
}
