...
class RecipientTypeUserGroup implements RecipientInterface
{
    ...
    /**
     * RecipientTypeUserGroup constructor.
    ...
    {
        return NotificationRecipients::TYPE_USER_GROUPS_LIST;
    }
    ...
    public function getRecipients(): array
    {
        ...
        if (0 !== \count($ids)) {
            $userGroups = $this->userGroupRepository
                ->findBy(
                    ['team' => $team, 'id' => $ids, 'status' => [User::STATUS_ACTIVE, User::STATUS_NEW]]
                );

            $users = [];
            foreach ($userGroups as $userGroup) {
                $usersFromGroup = $userGroup->getUsersEntityArray();
                $users = array_merge($users, $usersFromGroup);
            }
