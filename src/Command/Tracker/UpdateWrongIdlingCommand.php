...
#[AsCommand(name: 'app:tracker:update-wrong-idling')]
class UpdateWrongIdlingCommand extends Command
{
    use BreakableTrait;
    use RedisLockTrait;
    use MemorableTrait;
    ...
    {
        $this->setDescription('Update wrong idling');
        ...
        try {
            ...
            foreach ($deviceIds as $deviceId) {
                ...
                $thtCount = $this->em->getRepository(TrackerHistoryTemp::class)
                    ->updateTrackerHistoriesTempAsNotCalculatedForIdling(
                        $deviceId,
                        $startedAt,
                        $finishedAt
                    );
            ...
            $progressBar->finish();
            $output->writeln(PHP_EOL . 'Wrong idling successfully updated!');
